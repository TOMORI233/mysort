...
% NOTICE: It ONLY works with single-channel data.
% Principle:
%     Assume that data0 and data1 are different recorded protocol data from one cell
%     or from one long-term recording data.
%     We are to sort data0 with mysort to generate spike waveform templates and apply
%     template matching to data1.
%     The template matching algorithm is based on sum of sqaure error (SSE).
%     The basic principal is that the amplitude of spikes from the same cell follows
